#!/usr/bin/env bash
# run_discover_once.sh — run discover_once.py once using .env

set -euo pipefail
cd /home/ytscan/yt-autoscanner

# Tránh lỗi CRLF nếu file tạo từ Windows
# sed -i 's/\r$//' run_discover_once.sh  # (chỉ cần chạy 1 lần bên ngoài)
...
# Gán khối text bằng cat <<EOF để lệnh trả về 0
export YT_RANDOM_QUERY_POOL="$(cat <<'EOF'
...
echo "YT_DURATION_MODE=$YT_DURATION_MODE"

export YT_MAX_PAGES="10"

# Run once (uses .env for MONGO_URI, YOUTUBE_API_KEY)
source venv/bin/activate
python worker/discover_once.py | tee -a logs/discover_manual_$(date +%Y%m%d_%H%M%S).log