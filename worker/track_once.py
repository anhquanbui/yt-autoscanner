...
#   - Early-stop priority:
#       1) If low_quality_v1_3h.is_low == True/1 → stop_reason="ml.low_quality_v1_3h"
#       2) Else if low_quality_v3_6h.is_low == True/1 → stop_reason="ml.low_quality_v3_6h"
...
def main() -> int:
    ...
    print(
        f"Plan milestones (first 8): {PLAN_MINUTES[:8]}"
    ...
    try:
        ...
        if str(reason) in quota_reasons:
            print(
                "YouTube quota exhausted during duration backfill — continuing stats only.",
                file=sys.stderr,
            )
        else:
            print(
    ...
    if LOG_SAMPLE and due_docs:
        ...
        for d in due_docs[:LOG_SAMPLE]:
            print(
                f" - {d['_id']} | prev next_poll_after="
                f"{d.get('tracking', {}).get('next_poll_after')}"
            )
