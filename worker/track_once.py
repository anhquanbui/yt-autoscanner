...
#   - Reused load_dotenv from config.path_utils for consistent .env loading.
...
# Centralized .env loader with safe fallback
# (path_utils internally handles missing python-dotenv)
from config.path_utils import load_dotenv  # type: ignore[import]
...
# (idempotent; safe to call again later if needed)
load_dotenv()

# ----- Static config from env (used as global defaults) -----
...
def run_discover(
    *,
    ...
    # --- Read env defaults ---
    env_region = os.getenv("YT_REGION", "US")
    env_query = os.getenv("YT_QUERY")

    env_random_pick = os.getenv("YT_RANDOM_PICK", "0").lower() in ("1", "true", "yes")
    env_random_region_pool_raw = os.getenv("YT_RANDOM_REGION_POOL", "")

    env_since_minutes = int(os.getenv("YT_SINCE_MINUTES", "20"))
    env_max_pages = int(os.getenv("YT_MAX_PAGES", "1"))

    env_duration_mode = os.getenv("YT_DURATION_MODE", "any").lower()
    env_duration_pool = os.getenv(
        "YT_DURATION_POOL", "short:1,medium:1,long:1,any:0"
    )

    env_exclude_live = os.getenv("YT_EXCLUDE_LIVE", "1").lower() in (
        "1",
        "true",
        "yes",
    )
