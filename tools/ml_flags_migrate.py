...
#
# Dùng cho môi trường VPS:
#   - Đọc cấu hình từ /home/ytscan/.env (nếu có) và ./.env
#   - Mặc định DB: ytscan, collection: videos
#   - Chạy idempotent, có --dry-run để xem trước.
...
try:
    # Load env từ /home/ytscan/.env (VPS) và ./.env (local) nếu có
    from dotenv import load_dotenv

    _HOME_ENV = "/home/ytscan/.env"
    if os.path.exists(_HOME_ENV):
        load_dotenv(_HOME_ENV)
    if os.path.exists(".env"):
        load_dotenv(".env")
except Exception:
    # Không bắt buộc phải có python-dotenv để chạy
    pass
...
def main() -> int:
    ...
    try:
        ...
        # Trigger a server selection to fail fast if URI sai
