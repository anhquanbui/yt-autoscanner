...
function Ensure-BackgroundWorker {
    ...
    try {
        # If previous process exists and still running → skip to avoid overlap
        if ($ProcVar.Value -and (-not $ProcVar.Value.HasExited)) {
            Write-Log "$Name is still running (PID=$($ProcVar.Value.Id)), skip this tick." "INFO"
            return
        }
        ...
        $ProcVar.Value = $proc
        Write-Log "$Name started (PID=$($proc.Id)) in background."

    } catch {
        Write-Log "Exception while starting background worker ${Name}: $($_.Exception.Message)" "ERROR"
    }
}
...
while ($true) {
    ...
    # ---- Foreground workers (block while running) ----
    if ($now -ge $NextDiscover) {
        Run-ForegroundStep -Name "discover_once" -ScriptRelPath "discover_once.py"
        $NextDiscover = $now.AddSeconds($DiscoverIntervalSeconds)
    }

    if ($now -ge $NextTrack) {
        Run-ForegroundStep -Name "track_once" -ScriptRelPath "track_once.py"
    ...
    if ($now -ge $NextKpi) {
        Ensure-BackgroundWorker -Name "compute_dashboard_kpis" `
